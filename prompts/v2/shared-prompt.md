...
- `russians` means Russian national or ethnic identity;
  `russian_speaking_ukrainians` means Ukrainians represented through their use
  of Russian. The message's language alone is not evidence for either group.
...
- Distinguish current `military_personnel`, specific `tcc_personnel`, former
  `veterans`, and people explicitly described as `demobilized`. Do not add a
  broader group automatically when only a subtype is discussed.
